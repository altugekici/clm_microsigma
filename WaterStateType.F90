...
!hl
     real(r8), pointer :: excess_ice_col         (:,:) ! col excess ice lens/(kg/m2)  (-nlevsno+1:nlevgrnd)
     real(r8), pointer :: init_exice             (:,:) ! initial value of col excess ice lens/(kg/m2)
     real(r8), pointer :: exice_melt             (:)   ! column excess ice melting (m) 
!hl2
   contains
...
!hl: add grlnd
    use clm_varcon      , only : h2osno_max, zlnd, tfrz, spval, pc, grlnd
...
!hl,scs
    ...
    ! Open surface dataset to read in data below
      fexice = '/home/uni/aekici/data/ipa_nsidc_excessice_ave.nc'

      call getfil (fexice, locfn, 0)
      ...
      if (.not. readvar) then
         if (masterproc) then
            write(iulog,*) 'excess_ice not found on surface data set.'
...
!AE : restart exice and microsgima values
