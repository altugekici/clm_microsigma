...
contains
  ...
  subroutine SoilTemperature(bounds, num_urbanl, filter_urbanl, num_nolakec, filter_nolakec, &
    ...
    use clm_varctl               , only : iulog
!hl: add denice to clm_varcon
    use clm_varcon               , only : cnfac, cpice, cpliq, denice, denh2o
...
!hl
    ...
    associate(                                                                   & 
!hl
         excess_ice              => waterstate_inst%excess_ice_col          , & ! Input:  [real(r8) (:,:) ]  col excess ice lens/(kg/m2)  (-nlevsno+1:nlevgrnd)
         init_exice              => waterstate_inst%init_exice              , & ! Input:  [real(r8) (:,:) ]  initial value of col excess ice lens/(kg/m2)
!hl
...
!hl: call in Phasechange_excess_ice 
...
!hl: use excess ice derived soil properties
...
!hl: Phasechange_excess_ice
  ...
  subroutine Phasechange_excess_ice (bounds, num_nolakec, filter_nolakec, dhsdT, &
    ...
    use clm_varpar       , only : nlevsno, nlevgrnd,nlevurb
    use clm_varctl       , only : iulog
!hl: denice included in clm_varcon
...
!hl
    ...
    associate(                                                        & 
!hl
         excess_ice              => waterstate_inst%excess_ice_col  , & ! Inout:  [real(r8) (:,:) ]  col excess ice lens/(kg/m2) (-nlevsno+1:nlevgrnd)
         init_exice              => waterstate_inst%init_exice      , & ! Input:  [real(r8) (:,:) ]  initial value of col excess ice lens/(kg/m2)
         micro_sigma             => col%micro_sigma  ,                & ! Inout:  [real(r8) (:)   ]  microtopography change from excess ice melting (m)
         exice_melt              => waterstate_inst%exice_melt      , & ! Inout:  [real(r8) (:)   ]  column excess ice melting (m) 
!hl
         snl              =>    col%snl                             , & ! Input:  [integer  (:)   ] number of snow layers                    
         dz               =>    col%dz                              , & ! Input:  [real(r8) (:,:) ] layer thickness (m)                    
...
!hl: excess ice added in the calculation of melting
...
!hl
 ...
 !hl(scs): melting block, xm should only be non-negative if h2osoi_ice and/or excess
 !ice exist
 ...
 !hl(scs): case where h2osoi_ice is zero, but excess_ice > 0
                      xm2(c,j) = xm(c,j)
 
                      if (xm2(c,j) > excess_ice(c,j)) then
 !#9
 !hl(scs): melt all excess_ice, and calculate heatr, heat liquid
                         xm3(c,j) = xm2(c,j) - excess_ice(c,j)
                         heatr = hfus*xm3(c,j)/dtime
                         excess_ice(c,j) = 0._r8
                      elseif (xm2(c,j) <= excess_ice(c,j)) then
 !#10
 !hl(scs): melt some excess_ice, stop, set heatr = 0
                         excess_ice(c,j) = excess_ice(c,j)-xm2(c,j)
                         heatr = 0._r8
                         xm3(c,j) = 0._r8
                      endif
 ...
 !hl(scs): need to add heatr calculation here
                      heatr = hm(c,j) - hfus*(wice0(c,j)-h2osoi_ice(c,j))/dtime
 !hl(scs)
 !hl(scs): This part was modified for the urban problem
                   else
 ...
 !hl(scs): excess ice added in heatr calculations
 !                    heatr = hm(c,j) - hfus*(wice0(c,j)-h2osoi_ice(c,j))/dtime
                   endif
                endif
...
!hl: end of original code
                     ...
                     if (j >= 1) then 
!hl(scs): add excess ice in xmf calculations
...
!hl2   compute exice_melt for each column (in meters)
         exice_melt(c) = SUM( init_exice(c,:) - excess_ice(c,:) ) / denice
!hl2
         ! recompute micro sigma since exice_melt changes over time
         if (exice_melt(c)<0.5) then
            micro_sigma(c) = max(0.0001,(col%topo_slope(c) + slope0)**(- slopebeta) - exice_melt(c)/10.0)
         else
            micro_sigma(c) = (col%topo_slope(c) + slope0)**(- slopebeta) + exice_melt(c)/10.0
         end if
      end do
         ...
         do fc = 1,num_nolakec
            ...
            if ((col%itype(c) == icol_sunwall .or. col%itype(c) == icol_shadewall &
               ...
               if (j >= col%snl(c)+1) then
                 ...
                 .and. col%itype(c) /= icol_roof) then
!hl: use excess ice derived layer properties
               if (j >= col%snl(c)+1) then
                  if (j == col%snl(c)+1) then
